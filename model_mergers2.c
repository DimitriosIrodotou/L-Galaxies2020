 ...
 *         a bulge. New stars form of to the bulge*/
	...
	if (DiskInstabilityModel == 0) {
		if (mass_ratio < ThreshMajorMerger && (Gal[merger_centralgal].DiskMass + Gal[merger_centralgal].BulgeMass) > 0.0) {
			...
			check_disk_instability_DI(merger_centralgal, deltaT/STEPS);
...
void bulgesize_from_merger (double mass_ratio, int merger_centralgal, int p,
	...
	if (mass_ratio < ThreshMajorMerger) {
		...
		if (Mcen > 0.0) {
			Rcen = (RStellarDiskCentral * 1.68 * (MstarCentral - MbulgeCentral) + Gal[merger_centralgal].BulgeSize * MbulgeCentral +
			        RgasCentral * frac * MgasCentral * 1.68) / (MgasCentral * frac + MstarCentral);
		} else {
			Rcen = 0.0;
		}
		// and satellite Msat
		...
		if (Msat > 0.0) {
			Rsat = (RStellarDiskSat * 1.68 * (MstarSat - MbulgeSat) + Gal[p].BulgeSize * MbulgeSat + RgasSat * frac * MgasSat * 1.68) /
			       (MgasSat * frac + MstarSat);
		} else {
			Rsat = 0.0;
		}
	}
	...
	if (Rcen >= tiny && Rsat >= tiny) {
		Gal[merger_centralgal].BulgeSize =
				(Msat + Mcen) * (Msat + Mcen) / (Msat * Msat / Rsat + Mcen * Mcen / Rcen + fint / c * Msat * Mcen / (Rsat + Rcen));
	} else if (Rcen >= tiny) {
