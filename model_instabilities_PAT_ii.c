...
void check_disk_instability_DI (int p, double dt) {
	...
	// Star a loop from ring 0 to ring 10 and check the stability of each ring.
	for (j = 0; j < RNUM -1; j ++) {
...
void fix_gas_instabilities (int p, int j, double SigmaGasRings, double SigmaCritGas, double fmig) {
	double UnstableMass, fbulge, fcarry = 0.1;
...
void fix_stellar_instabilities (int p, int j, double SigmaStarRings, double SigmaCritStars) {
	double UnstableMass, fbulge, fcarry = 0.3;
	...
	if (UnstableMass > 0.0) {
		...
		// Remove unstable stars from the ring: fbulge of it goes from the ring/disk to the bulge and the remaining (1 - fbulge) to ring j-1.
		Gal[p].DiskMassRings[j] -= UnstableMass;
		Gal[p].DiskMass -= fbulge * UnstableMass;
		Gal[p].DiskMassRings[j + 1] += (1 - fbulge) * UnstableMass;

		Gal[p].BulgeMass += fbulge * UnstableMass;
...
void update_sizes_from_instabilities (int p, int j, double UnstableMass, double fcarry, char process[]) {
	...
	// Since the mass of the disk that goes into the bulge carries a fraction (fcarry) of disk's sAM then there is no need to update its size/AM.
	// But bulge's AM has to increase.
	if (strcmp (process, "Subtract") == 0) {
		frac = UnstableMass / ((2 - fcarry) * Gal[p].DiskMass);

		for (ii = 0; ii < 3; ii ++) {
			Gal[p].BulgeSpin[ii] += fcarry * frac * Gal[p].DiskSpin[ii];
		}
