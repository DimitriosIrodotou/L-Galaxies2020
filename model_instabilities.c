...
void fix_stellar_instabilities (int p, int j, double SigmaStarRings, double SigmaCritStars) {
	...
	if (UnstableMass > 0.0) {
		...
		if (Gal[p].BulgeMass > 0.0 && Gal[p].BulgeSize > 0.0) {
			if (RingRadius[j] > Gal[p].BulgeSize) { // Move 2/3 of the unstable mass to the inner and 1/3 to the outer ring along with the associated
				// metals.
...
//			update_metals_from_instabilities (p, j, UnstableMass, "DiskMass", "BulgeMass");
			...
			if (j > BulgeThresh) { // Move 2/3 of the unstable mass to the inner and 1/3 to the outer ring along with the associated
				// metals.
				Gal[p].DiskMassRings[j - 1] += (2 * UnstableMass) / 3.;
				Gal[p].DiskMassRings[j + 1] += UnstableMass / 3.;
				Gal[p].DiskMassRings[j] -= UnstableMass;
